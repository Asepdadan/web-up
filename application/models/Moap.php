...
class Moap extends CI_Model {
    ...
    */
        ...
        public function csv($q,$name = 'CSV'){
            $this->load->dbutil();
            ...
            force_download($name.'_versi1.csv', $data);
        }
        
        public function csv2($q,$name = 'CSV'){
            $this->load->dbutil();
            $this->load->helper('file');
            $this->load->helper('download');
            $query = $this->db->query($q);
            ...
            $newline = "\r\n";
            $data = $this->dbutil->csv_from_result($query, $delimiter, $newline);
            force_download($name.'_versi2.csv', $data);
        }
        ...
        public function Test(){
            echo "Mamah mia";
            $query = $this->db->query("select * from mst_provinsi");
            foreach ($query->list_fields() as $item) {
                echo "Hai $item";
            }               
        }
